...
import (
	...
	ct"github.com/icloudland/starchain/core/contract"
	"sync"
	"time"
	"fmt"
	"os"
	...
	"github.com/icloudland/starchain/events/signalset"
	"crypto/sha256"
	"strings"
	"github.com/icloudland/starchain/common/passwd"
	"sort"
	"encoding/hex"
	"errors"
	"math/rand"
...
func (cl *ClientImpl) GetDefaultAccount() (*Account, error) {
	...
	if acc != nil {
		...
		address,_:=cl.mainAccount.ToAddress()
		log.Error("can't find acc :",address)
		return nil,errors.New("can't find account")
	}
}

func (cl *ClientImpl) GetAccount(pubKey *crypto.PubKey) (*Account, error) {
	signatureRedeemScript, err := ct.CreateSignatureRedeemScript(pubKey)
	if err != nil {
...
func (cl *ClientImpl) SaveAccount(ac *Account) error {
	...
	// save Account keys to wallet file
...
func (cl *ClientImpl) CreateContract(account *Account) error {
	contract, err := ct.CreateSignatureContract(account.PubKey())
	if err != nil {
		return err
	}
...
func (cl *ClientImpl) CreateMultiSignContract(contractOwner Uint160, m int, publicKeys []*crypto.PubKey) error {
	contract, err := ct.CreateMultiSigContract(contractOwner, m, publicKeys)
	if err != nil {
		return err
	}
	if err := cl.SaveContract(contract); err != nil {

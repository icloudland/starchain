...
import (
	...
	"github.com/icloudland/starchain/net/websocket/session"
	"github.com/icloudland/starchain/net/httprestful/ErrorCode"
	"context"
	"errors"
...
func (ws *WsServer) checkSessionsTimeout(done chan bool) {
	...
	for {
		...
		case <-ticker.C:
			...
			ws.SessionList.ForEachSession(func(v *session.Session) {
				if v.SessionTimeoverCheck() {
					resp := ResponsePack(ErrorCode.SESSION_EXPIRED)
...
func (ws *WsServer) OnDataHandle(curSession *session.Session, bysMsg []byte, r *http.Request) bool {
	...
	if err := json.Unmarshal(bysMsg, &req); err != nil {
		resp := ResponsePack(ErrorCode.ILLEGAL_DATAFORMAT)
	...
	if !ok {
		resp := ResponsePack(ErrorCode.INVALID_METHOD)
		ws.response(curSession.GetSessionId(), resp)
		return false
	}
	...
	if !ws.IsValidMsg(req) {
		resp := ResponsePack(ErrorCode.INVALID_PARAMS)
		ws.response(curSession.GetSessionId(), resp)
		return true
	}
